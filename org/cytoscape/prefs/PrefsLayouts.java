...
public class PrefsLayouts extends MasterDetailPanel {
	...
	// String[] layoutsx = {"attribute-circle", "attributes-layout", "circular",
	// "cose", "degree-circle", "force-directed-cl", "force-directed",
	// "fruchterman-rheingold", "grid", "hierarchical", "isom", "kamada-kawai",
	// "stacked-node-layout" };
	//
	...
	private void makeLayoutList() {
		...
		addLayout("Edge-weighted Spring-Embedded", "kamada-kawai", makeEdgeWeightedSpringEmbeddedLayoutEditor(),
		...
		addLayout("Prefuse Force Directed", "force-directed", makePrefuseForceLayoutEditor(),
				"#prefuse-force-directed-layout");
		addLayout("Prefuse Force Directed OpenCL", "force-directed-cl", makePrefuseForceOpenCLLayoutEditor(),
				"#prefuse-force-directed-layout");
	}
	...
	private NestedPrefPanel makeEdgeWeightedForceLayoutEditor() {
		...
		HBox lineA = new HBox(panel.makeNumberFieldShort("Attraction", 70, "attraction_multiplier", 5, 1, 100));
		HBox lineB = new HBox(panel.makeNumberFieldShort("Repulsion", 80, "repulsion_multiplier", 6, 1, 100));
		HBox lineC = new HBox(panel.makeNumberFieldShort("Gravity", 60, "gravity_multiplier", 2, 1, 100));
		HBox lineC2 = new HBox(panel.makeNumberFieldShort("Compound Gravity", 130, "gravity_multiplier", 6, 1, 100));
		page.add(new HBox(false, true, lineA, lineB));
		page.add(new HBox(false, true, lineC, lineC2));
		//
		HBox lineM = new HBox(panel.makeNumberFieldShort("Conflict Avoidance", 120, "conflict_avoidance", 4, 1, 100));
		HBox lineN = new HBox(panel.makeNumberFieldShort("Temperature", 80, "temperature", 6, 1, 100));
		...
		HBox lineR = new HBox(panel.makeNumberFieldShort("Extra room", 80, "extra_room", 1, 1, 100));
	...
	private NestedPrefPanel makeEdgeWeightedSpringEmbeddedLayoutEditor() {
		...
		HBox bLine = panel.makeNumberFieldShort("Discon. strength", 140, "m_disconnectedNodeDistanceSpringStrength", 1,
				1, 100);
		bLine.add(
				panel.makeNumberFieldShort("rest length", 70, "m_disconnectedNodeDistanceSpringRestLength", 1, 1, 100));
		page.add(aLine);
		page.add(bLine);
	...
	private NestedPrefPanel makeGroupLayoutEditor() {
		...
		page.add(panel.makeSpacingBox());
		page.add(Box.createRigidArea(lineSpace));
		page.add(panel.makeNumberField("Maximum Row Width", "maxwidth", 1000, 100, 10000));
		page.add(panel.makeNumberField("Minimum Partition Width", "minrad", 100, 1, 1000));
		page.add(panel.makeNumberField("Scale of the Partition Radius", "radmult", 3, 1, 100));
		page.add(Box.createRigidArea(lineSpace));
		return panel;
	}
...
//	map.stream().filter().collect();
	...
	private NestedPrefPanel makePrefuseForceOpenCLLayoutEditor() {
		...
		// page.add(makeNumberField("Spring Coefficient", "force-directed-cl",
		// "", 1, 1, 100));
		// page.add(makeNumberField("Spring Length", "force-directed-cl", "", 1,
		// 1, 100));
	...
	HBox makeEdgeWeightLine() {
		...
		HBox line2 = new HBox(true, true, new JLabel("Min"), numberField(1, 100), new JLabel("Max"), numberField(1000, 100000));
		line2.add(new JLabel("Default"));
		line2.add(numberField(100, 1000));
