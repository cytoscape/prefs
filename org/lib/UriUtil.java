...
public class UriUtil {
	...
	public static String extractEngineFromFJRef(String fjref)
	{
		String identifier = StringUtil.ltrim(fjref, FJ_REMOTE_REF_SCHEME_PREFIX);
		int index = identifier.indexOf(FJ_ENGINE_PATH_COMPONENT);
		if (index > 0)
			identifier = identifier.substring(0, index);
		return identifier;
	}

	public static String buildFJRef(String hostEngine, String reference)
	{
		// if host engine starts with http or https, strip it before adding fjref:
		if (hostEngine.startsWith(UriUtil.HTTPS_SCHEME))
			hostEngine = hostEngine.substring(UriUtil.HTTPS_SCHEME.length());
		else if (hostEngine.startsWith(UriUtil.HTTP_SCHEME))
			hostEngine = hostEngine.substring(UriUtil.HTTP_SCHEME.length());
		if (hostEngine.startsWith("://"))
			hostEngine = hostEngine.substring("://".length());
		return FJ_REMOTE_REF_SCHEME_PREFIX + hostEngine + URI_SLASH + reference;
	}
	public static String stripFJRemoteRef(String remoteRef)
	{
		if (!remoteRef.startsWith(FJ_REMOTE_REF_SCHEME_PREFIX)) return remoteRef;
		remoteRef = remoteRef.substring(FJ_REMOTE_REF_SCHEME_PREFIX.length());
		int index = remoteRef.lastIndexOf(URI_SLASH); // indexOf(URI_SLASH); // now for servlet name
		if (index > 0)
			remoteRef = remoteRef.substring(index+1);
		return remoteRef;
	}
	public static String stripFileRef(String str)
	{
    	...
    	if (HomeEnv.isWindows() && str.charAt(0) == '/')
    		str = str.substring(1);
    	return str;
	}
	...
	public static String fileExtFromUri(URI uri) {
		return FileUtil.extFromFileName( nameFromUri( uri ) );
	}
